@extends('layouts.admin', ['title' => 'Edit Sales Quotation'])
...
<div class="space-y-6" x-data="quotationBuilder()">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-3xl font-extrabold text-white tracking-tight">Edit Quotation #{{ $quotation->morder_id }}</h1>
            <p class="text-sm text-slate-400">Update estimate, proforma invoice, or sales proposal draft details.</p>
        </div>
    </div>
    ...
    <form action="{{ route('admin.quotations.update', $quotation->order_id) }}" method="POST" enctype="multipart/form-data" x-on:submit="handleSubmit($event)" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="p-6 glassmorphism rounded-2xl space-y-4">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div class="space-y-1.5">
                            ...
                            <input type="text" 
                                   ...
                                   class="w-full px-4 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-slate-200 placeholder-slate-660 focus:outline-none focus:border-indigo-500 text-sm uppercase">
                        </div>
                        ...
                        <div class="space-y-1.5">
                            ...
                            <input type="date" 
                                   id="orderDate" 
                                   name="orderDate" 
                                   value="{{ old('orderDate', date('Y-m-d', strtotime($quotation->order_date))) }}" 
                                   required 
                                   class="w-full px-4 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-slate-200 focus:outline-none focus:border-indigo-500 text-sm">
                        </div>
                        ...
                        <div class="space-y-1.5">
                            ...
                            <input type="text" 
                                   ...
                                   class="w-full px-4 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-slate-200 placeholder-slate-660 focus:outline-none focus:border-indigo-500 text-sm">
                        </div>
                        ...
                        <div class="sm:col-span-2 space-y-1.5">
                            ...
                            <textarea id="clientContact" 
                                      ...
                                      class="w-full px-4 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-slate-200 placeholder-slate-660 focus:outline-none focus:border-indigo-500 text-sm uppercase"></textarea>
                        </div>
                    </div>
                </div>
                                        ...
                                        <td class="py-2 lg:pl-2 col-span-2 block lg:table-cell lg:col-span-none">
                                            ...
                                            <div class="flex gap-2">
                                                <select name="productName[]" 
                                                    ...
                                                    <template x-if="item.productName && !products.some(p => p.productname.toUpperCase() === item.productName.toUpperCase())">
                                                        <option :value="item.productName.toUpperCase()" x-text="item.productName + ' (Custom/Legacy)'" ></option>
                                                    </template>
                                                    <template x-for="p in products" :key="p.id">
                                                        <option :value="p.productname.toUpperCase()" x-text="p.productname" :selected="item.productName === p.productname.toUpperCase()"></option>
                                                    </template>
                                                </select>
                                            </div>
            ...
            <div class="space-y-6">
                <!-- Summary Card -->
                <div class="p-6 glassmorphism rounded-2xl space-y-6 border border-slate-800/60 lg:sticky lg:top-24">
                    ...
                    <div class="space-y-4 text-xs font-bold text-slate-400 uppercase tracking-wider">
                        <div class="space-y-1.5">
                            ...
                            <select id="qtype" 
                                ...
                                <option value="3" {{ old('qtype', $quotation->qtype) == 3 ? 'selected' : '' }}>Sales Quotation</option>
                                <option value="1" {{ old('qtype', $quotation->qtype) == 1 ? 'selected' : '' }}>Proforma Invoice</option>
                                <option value="2" {{ old('qtype', $quotation->qtype) == 2 ? 'selected' : '' }}>Cost Estimate</option>
                            </select>
                        </div>
                        ...
                        <div class="space-y-1.5">
                            ...
                            <select id="qstate" 
                                    name="qstate" 
                                    class="w-full px-4 py-2.5 bg-slate-950 border border-slate-850 rounded-xl text-slate-200 focus:outline-none focus:border-indigo-500 font-semibold normal-case">
                                <option value="1" {{ old('qstate', $quotation->qstate) == 1 ? 'selected' : '' }}>Active / Pending Approval</option>
                                <option value="2" {{ old('qstate', $quotation->qstate) == 2 ? 'selected' : '' }}>Draft</option>
                            </select>
                        </div>
                        ...
                        <div class="space-y-1.5">
                            ...
                            @if ($quotation->signa)
                                <div class="mb-2 p-2 bg-slate-900 border border-slate-800 rounded-lg flex items-center justify-between">
                                    <div class="flex items-center gap-2">
                                        <i class="fa-solid fa-signature text-slate-400"></i>
                                        <span class="text-xs text-slate-400 font-mono truncate max-w-[150px]">{{ $quotation->signa }}</span>
                                    </div>
                                    <span class="text-[10px] text-emerald-400 font-bold uppercase">Uploaded</span>
                                </div>
                            @endif
                            <input type="file" 
                                   id="signature" 
...
<script>
    function quotationBuilder() {
        return {
            ...
            clientSelection: '0',
            client: {
                name: @json(old('clientName', $quotation->client_name)),
                mobile: @json(old('mobileno', $quotation->mobile)),
                gsttin: @json(old('gsttin', $quotation->gsttin)),
                address: @json(old('clientContact', $quotation->client_contact))
            },
            items: [],
            financials: {
                subTotal: 0,
                tax: 0,
                shipping: {{ old('shipcharge', $quotation->shipamt ?? 0) }},
                interest: {{ old('intcharge', $quotation->instamt ?? 0) }},
            ...
            init() {
                // Populate existing items from PHP
                const existingItems = @json($quotation->items);
                if (existingItems && existingItems.length > 0) {
                    this.items = existingItems.map(item => ({
                        productName: item.product_id, // string product name in legacy database
                        qty: parseFloat(item.qty) || 1,
                        rate: parseFloat(item.rate) || 0,
                        gst: parseInt(item.gst) || 0,
                        unit: item.unit || 'PCS',
                        hsnsac: item.hsnsan || '',
                        wgst: parseFloat(item.gstr) || 0,
