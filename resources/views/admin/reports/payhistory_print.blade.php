...
<head>
    ...
    <title>Customer Ledger Statement - MTL Mart</title>
    <!-- Fonts -->
    ...
    <div class="max-w-6xl mx-auto my-8 p-6 print:m-0 print:p-0">
        <div class="print-sheet bg-white border border-slate-200 shadow-xl rounded-3xl p-10 print:bg-white print:border-none print:shadow-none print:rounded-none">
            ...
            <div class="flex justify-between items-center my-6 text-xs bg-slate-50 p-4 rounded-2xl border border-slate-200/80">
                <div>
                    <span class="block text-[10px] font-extrabold text-slate-400 uppercase tracking-widest">Financial Ledger Profile</span>
                    <h2 class="text-base font-bold text-slate-900 font-outfit uppercase">
                        {{ strtoupper($user->uname) }}
                    </h2>
                    <p class="text-slate-500 text-[11px] mt-0.5">
                        Address: {{ $user->billingaddress ?? 'N/A' }}, {{ $user->billingcity ?? 'N/A' }} | Mob: {{ $user->contactno ?? 'N/A' }}
                    </p>
                </div>
                ...
                <table class="w-full text-left text-xs border-collapse">
                    <thead>
                        <tr class="bg-slate-50 border-y border-slate-200 text-slate-600 font-bold uppercase tracking-wider">
                            ...
                            <th class="py-3 px-3 w-20 text-center">Bill No</th>
                            <th class="py-3 px-3">Transaction / Payee</th>
                            <th class="py-3 px-3">Address / Area</th>
                            <th class="py-3 px-3 w-24 text-center">Date</th>
                            <th class="py-3 px-3 w-24 text-right">Grand Total</th>
                            <th class="py-3 px-3 w-24 text-right">Paid Install</th>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        ...
                        @forelse ($ledgerData['ledger'] as $record)
                            @php
                                ...
                                $clientName = $user->uname;
                                $clientAddress = $user->billingaddress . ", " . ($user->billingcity ?? '');
                                if ($user->contactno) {
                                    $clientAddress .= " Mob: " . $user->contactno;
                                }
                            ...
                            @empty
                                ...
                                <tr class="text-slate-700 align-middle">
                                    ...
                                    <td class="py-2.5 px-3 font-semibold text-slate-900 uppercase">{{ $clientName }}</td>
                                    <td class="py-2.5 px-3 text-xs text-slate-500 uppercase">{{ $clientAddress }}</td>
                                    <td class="py-2.5 px-3 text-center font-mono text-slate-650 font-medium">{{ $orderDate }}</td>
                                    <td class="py-2.5 px-3 text-right font-mono">Rs. {{ number_format($isOnline ? $order->gamount : $order->grand_total, 2) }}</td>
                                    <td class="py-2.5 px-3 text-right font-mono text-emerald-600 font-semibold">Rs. 0.00</td>
                                    <td class="py-2.5 px-3 text-right font-mono text-rose-600 font-bold">Rs. {{ number_format($isOnline ? $order->bamount : $order->due, 2) }}</td>
                                </tr>
                            @endforelse
                        ...
                        @empty
                            <tr>
                                <td colspan="8" class="py-8 text-center text-slate-500 font-medium">No transaction ledgers associated with this partner.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 my-8 pt-6 border-t border-slate-200 text-xs">
                ...
                <div class="space-y-4">
                    ...
                    <div class="text-slate-500 space-y-1">
                        ...
                        <p>This document lists all active payment cycles, installations, and credit limits assigned to the partner.</p>
                    </div>
                </div>
            ...
            <div class="flex justify-between items-center mt-12 border-t border-slate-200 pt-6 text-[10px] font-bold text-slate-500 uppercase tracking-wider">
                <div>MTL Mart Financial Ledger Statement</div>
