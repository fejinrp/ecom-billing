...
<head>
    ...
    <title>Sales Report - MTL Mart</title>
    <!-- Fonts -->
...
<body class="min-h-screen text-slate-800 antialiased print:bg-white print:text-slate-900">
    ...
    <div class="no-print bg-white/85 backdrop-blur-md border-b border-slate-200/80 sticky top-0 z-50 px-6 py-4 flex items-center justify-between shadow-sm">
        ...
        <div class="flex items-center gap-2">
            <button onclick="window.print()" class="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs uppercase tracking-wider shadow-sm transition">
    ...
    <div class="max-w-6xl mx-auto my-8 p-6 print:m-0 print:p-0">
        <div class="print-sheet bg-white border border-slate-200 shadow-xl rounded-3xl p-10 print:bg-white print:border-none print:shadow-none print:rounded-none">
            ...
            <div class="flex justify-between items-center my-6 text-xs bg-slate-50 p-4 rounded-2xl border border-slate-200/80">
                <div>
                    <span class="block text-[10px] font-extrabold text-slate-400 uppercase tracking-widest">Sales Category</span>
                    <h2 class="text-base font-bold text-slate-900 font-outfit uppercase">
                        {{ $catname }} SALES DETAILS
                    </h2>
                </div>
                ...
                <table class="w-full text-left text-xs border-collapse">
                    <thead>
                        <tr class="bg-slate-50 border-y border-slate-200 text-slate-600 font-bold uppercase tracking-wider">
                            ...
                            @if ($cat === 1 || $cat === 2)
                                <th class="py-3 px-3 w-16 text-center">B_No</th>
                            @endif
                            <th class="py-3 px-3 w-24 text-center">Date</th>
                            <th class="py-3 px-3">C_Name</th>
                            <th class="py-3 px-3">Address / Contact</th>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        ...
                        @forelse ($sales as $index => $sale)
                            @php
                                $saleId = ($cat === 1 || $cat === 2) ? $sale->order_id : $sale->orderid;
                                $invoiceNo = ($cat === 1 || $cat === 2) ? $sale->morder_id : $sale->morderid;
                                $orderDate = ($cat === 1 || $cat === 2) ? date('d-m-Y', strtotime($sale->order_date)) : date('d-m-Y', strtotime($sale->orderdate));
                                ...
                                // Add totals
                                $totalInst += ($cat === 1 || $cat === 2) ? $sale->instamt : $sale->install;
                                $totalShip += ($cat === 1 || $cat === 2) ? $sale->shipamt : $sale->tship;
                                $totalDis += $sale->discount;
                                $totalCoin += ($cat === 1 || $cat === 2) ? $sale->pcoin : 0;
                                $totalGrand += ($cat === 1 || $cat === 2) ? $sale->grand_total : $sale->gamount;
                                $totalPaid += ($cat === 1 || $cat === 2) ? $sale->paid : $sale->pamount;
                                $totalDue += ($cat === 1 || $cat === 2) ? $sale->due : $sale->bamount;
                            @endphp

                            @foreach ($items as $itemIdx => $item)
                                @php
                                    $productName = $item->product->productname ?? 'N/A';
                                    $qty = ($cat === 1 || $cat === 2) ? $item->qty : $item->quantity;
                                    $rate = ($cat === 1 || $cat === 2) ? $item->total : ($item->price * $item->quantity);
                                    $totalAmtt += $rate;
                                @endphp
                                <tr class="text-slate-700 align-middle">
                                    ...
                                    <!-- Customer Contact -->
                                    <td class="py-3 px-3 text-xs text-slate-500 uppercase">
                                        @if ($itemIdx === 0)
                                            <span class="max-w-[200px] truncate block" title="{{ $clientContact }}">{{ $clientContact }}</span>
                                        @endif
                                    </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="{{ ($cat === 1 || $cat === 2) ? 8 : 7 }}" class="py-8 text-center text-slate-500 font-medium">No sales recorded matching date range.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 my-8 pt-6 border-t border-slate-200 text-xs">
                ...
                <div class="space-y-4">
                    <div class="p-4 bg-slate-50 rounded-2xl border border-slate-200">
                        <span class="block text-[10px] font-extrabold text-slate-400 uppercase tracking-widest">Total Sales Count</span>
                        <p class="text-slate-800 font-bold mt-1 font-outfit text-base">
                            {{ count($sales) }} Verified Transactions
                        </p>
                    </div>
                    ...
                    <div class="text-slate-500 space-y-1">
                        ...
                        <p>This report contains verified sales entries matching the database system parameters recorded between specified calendar date bounds.</p>
                    </div>
                </div>
                ...
                <div class="space-y-3 bg-slate-50/50 p-6 rounded-2xl border border-slate-200/80 print:bg-white print:border-none print:p-0 print:space-y-2">
                    <div class="flex items-center justify-between text-slate-500">
                        ...
                        <span class="font-mono font-semibold text-slate-850">Rs. {{ number_format($totalAmtt, 2) }}</span>
                    </div>

                    <div class="flex items-center justify-between text-slate-500">
                        <span>Installation Amount:</span>
                        <span class="font-mono font-semibold text-slate-850">Rs. {{ number_format($totalInst, 2) }}</span>
                    </div>

                    <div class="flex items-center justify-between text-slate-500">
                        <span>Shipping Amount:</span>
                        <span class="font-mono font-semibold text-slate-850">Rs. {{ number_format($totalShip, 2) }}</span>
                    </div>

                    <div class="flex items-center justify-between text-slate-500 pt-2 border-t border-slate-200/60 font-semibold">
                        <span>Total Combined:</span>
                        <span class="font-mono text-slate-900">Rs. {{ number_format($totalAmtt + $totalInst + $totalShip, 2) }}</span>
                    </div>

                    <div class="flex items-center justify-between text-amber-600 font-semibold">
                        <span>Discount Allowed:</span>
                        <span class="font-mono font-bold">- Rs. {{ number_format($totalDis, 2) }}</span>
                    </div>

                    @if ($totalCoin > 0)
                        <div class="flex items-center justify-between text-indigo-600 font-semibold">
                            <span>Mcoin Redeemed:</span>
                            <span class="font-mono font-bold">- Rs. {{ number_format($totalCoin, 2) }}</span>
                        </div>
                    @endif

                    <div class="flex items-center justify-between text-sm font-bold text-slate-900 pt-3 border-t border-slate-200">
                        <span class="font-outfit text-base">Grand Total Sales:</span>
                        <span class="font-mono text-lg text-indigo-600">Rs. {{ number_format($totalGrand, 2) }}</span>
                    </div>
                    ...
                    <div class="pt-3 mt-1 border-t border-slate-200 space-y-1.5">
                        <div class="flex items-center justify-between text-xs text-emerald-600 font-semibold">
                            ...
                            <span class="font-mono text-sm">Rs. {{ number_format($totalPaid, 2) }}</span>
                        </div>
                        <div class="flex items-center justify-between text-xs text-rose-600 font-bold">
                            <span>Total Outstanding (Due):</span>
                            <span class="font-mono text-sm">Rs. {{ number_format($totalDue, 2) }}</span>
                        </div>
                    </div>
                </div>
            </div>
